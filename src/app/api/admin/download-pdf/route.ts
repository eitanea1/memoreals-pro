...
export async function GET(req: NextRequest) {
  ...
  for (const img of selectedImages) {
    ...
    // One image, scaled to COVER the whole media (card + bleed) keeping aspect.
    // The bleed is simply the image's proportional overflow past the cut line.
    ...
    for (let i = 0; i < 2; i++) {
      ...
      page.drawImage(embedded, {
        x: (PAGE_W - scaledW) / 2, y: (PAGE_H - scaledH) / 2, width: scaledW, height: scaledH,
      });
